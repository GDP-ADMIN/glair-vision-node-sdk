...
app.post("/general-document", async (req: Request, res: Response) => {
  const form = formidable({ multiples: true });
  const parse = (): Promise<any> => {
    return new Promise((resolve, reject) => {
      form.parse(req, async (err: any, fields: any, files: any) => {
  ...
  let error = null;
  const response = await vision()
    .ocr.generalDocument({ image: filePath })
    .catch((err) => (error = err));
...
app.post("/qualities", async (req: Request, res: Response) => {
  const form = formidable({ multiples: true });
  const parse = (): Promise<any> => {
    return new Promise((resolve, reject) => {
      form.parse(req, async (err: any, fields: any, files: any) => {
